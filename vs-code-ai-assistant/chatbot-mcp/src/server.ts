...
Only produce plain, normal code lines.`,
    ...
    this.app.post("/mcp", async (req: Request, res: Response) => {
      ...
      if (!this.toolsRegistered) {
        try {
          ...
          logger.error(
            `Failed to setup tools during request handling: ${error}`
          );
          if (!res.headersSent) {
            res.status(500).json({
              jsonrpc: "2.0",
  ...
  public async start(): Promise<void> {
    try {
      ...
      logger.info(
        `[MCPServer.start] Transport connected (took ${transportConnectTime}ms)`
      );
      ...
      try {
        ...
        logger.error(
          `[MCPServer.start] Error during tools setup: ${
      ...
      return new Promise((resolve, reject) => {
        ...
        this.httpServer = this.app.listen(this.port, "127.0.0.1", () => {
          ...
          logger.info(
            `[MCPServer.start] HTTP Server started (took ${httpStartTime}ms)`
          );
          logger.info(`MCP Server listening on localhost:${this.port}`);

          const totalTime = Date.now() - startTime;
          logger.info(
            `[MCPServer.start] Server startup complete (total: ${totalTime}ms)`
          );
        ...
        this.httpServer!.on("error", (error) => {
          logger.error(
            `[MCPServer.start] HTTP Server error during startup: ${error.message}`
          );
          reject(error);
        });
      ...
      logger.error(
        `[MCPServer.start] Failed to start MCP Server: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      throw error;
        ...
        await Promise.race([
          ...
          new Promise<void>((resolve, reject) => {
            this.httpServer!.close((err) => {
              ...
              if (err) {
                logger.error(
                  `[MCPServer.stop] HTTP server closed with error: ${err.message} (took ${httpCloseTime}ms)`
                );
                reject(err);
              } else {
                logger.info(
                  `[MCPServer.stop] HTTP server closed successfully (took ${httpCloseTime}ms)`
                );
                resolve();
              }
            });
          ...
          new Promise<void>((resolve) => {
            setTimeout(() => {
              logger.warn(
                `[MCPServer.stop] HTTP server close timed out after ${forceTimeout}ms - forcing close`
              );
